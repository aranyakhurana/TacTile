...
class Slider:
    def __init__(self, name, min_val, max_val, initial_val, position):
        self.name = name
        self.min_val = min_val
        self.max_val = max_val
        self.value = initial_val
        self.position = position  # (x, y) coordinates for the slider
    ...
    def draw(self, img):
        ...
        # Draw the slider bar
        cv2.rectangle(img, (x, y), (x + bar_width, y +
                      bar_height), (200, 200, 200), -1)

        # Calculate the width of the filled part based on current value
        fill_width = int((self.value - self.min_val) /
                         (self.max_val - self.min_val) * bar_width)
        cv2.rectangle(img, (x, y), (x + fill_width,
                      y + bar_height), (0, 255, 0), -1)

        # Display the slider name and value
        cv2.putText(img, f"{self.name}: {self.value}", (x, y - 10),
    ...
    def update_value(self, mouse_x):
        ...
        self.value = int(self.min_val + (mouse_x - x) /
                         bar_width * (self.max_val - self.min_val))
        # Clamp value within the slider range
...
if __name__ == '__main__':
    ...
    show_note_grid = True   # Toggle for displaying the note grid
    ...
    canvas_width = 780 + (2 * padding_offset)
    canvas_height = 390 + (2 * padding_offset)
    fixed_canvas = np.full((canvas_height, canvas_width, 3),
                           255, dtype=np.uint8)  # White background
    ...
    # Initialize sliders
    thresh_min_slider = Slider("Thresh Min", 0, 255, 15, (10, 450))
    thresh_max_slider = Slider("Thresh Max", 0, 255, 255, (10, 500))

    def draw_sliders(display_img):
        # Draw each slider on the image
        thresh_min_slider.draw(display_img)
        thresh_max_slider.draw(display_img)
    ...
    while True:
        ...
        # Overlay note grid
        display_img = overlay_note_grid(padded_img, note_grid, padding_offset)

        # Draw sliders on the display image
        display_img = draw_sliders(display_img)

        # Display the image with blobs in the OpenCV window
        cv2.imshow("Sensor Matrix", display_img)

        # Update the threshold parameters with slider values
        threshold_min = thresh_min_slider.value
        threshold_max = thresh_max_slider.value
