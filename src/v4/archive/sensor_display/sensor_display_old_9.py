...
def generate_image(data):
    ...
    padded_image = cv2.copyMakeBorder(
        resized_image, padding_offset, padding_offset, padding_offset, padding_offset, cv2.BORDER_CONSTANT, value=255)  # White border
...
def overlay_note_grid(display_img, note_grid):
    # Overlay the MIDI note grid on the OpenCV image
    rows, cols = len(note_grid.grid), len(note_grid.grid[0])
    cell_width = display_img.shape[1] // cols
    cell_height = display_img.shape[0] // rows
...
if __name__ == '__main__':
    ...
    padding_offset = 1  # This hack works for now, but make it 30 or higher for border padding; but incorporate scaling into the program
    ...
    while True:
        ...
        if show_note_grid:
            display_img = overlay_note_grid(display_img, note_grid)
